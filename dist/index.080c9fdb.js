...
document.addEventListener("click", ()=>{
    if (nav.classList.contains("show-nav")) nav.classList.remove("show-nav");
}, true);
...
faqContainers.forEach((faq)=>{
    faq.addEventListener("click", (e)=>{
        ...
        faqContainers.forEach((item)=>{
            if (item !== faq) item.classList.remove("show-faq");
        });
...
const stickyNav = function(entries) {
    const [entry] = entries;
    console.log(entry);
    if (!entry.isIntersecting) toTop.classList.add("show-to-top");
    else toTop.classList.remove("show-to-top");
};
...
function sendMail() {
    ...
    if (name.value === "" || mail.value === "") {
        ...
        emailjs.send("service_ropppqv", "template_wwmj8ua", params).then(function() {
...
//# sourceMappingURL=index.080c9fdb.js.map
